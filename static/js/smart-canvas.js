...
function bindNodeEvents(){
    world.querySelectorAll('.image-node').forEach(el => {
        ...
        el.ondrop = async e => {
            ...
            const workflowFile = dropFiles.find(item => /\.(json|zip)$/i.test(item.name || ''));
            if(workflowFile){
                importSmartWorkflowFile(workflowFile);
                return;
            }
...
function setSmartDropCopyEffect(e, includeAsset=false){
    ...
    const hasWorkflowFile = [...(e.dataTransfer?.files || [])].some(item => /\.(json|zip)$/i.test(item.name || ''));
    if(hasWorkflowFile || hasSmartImageDropData(e.dataTransfer) || (includeAsset && hasSmartAssetDrag(e.dataTransfer))){
...
async function runPromptLLMNode(nodeId){
    ...
    try {
        ...
        node.text = (result.content || result.text || '').trim();
        node.llmProvider = provider;
        node.llmModel = model;
        node.runStatus = 'done';
        if(!Array.isArray(canvas.logs)) canvas.logs = [];
        canvas.logs.unshift({
            status:'success', platform:provider || 'LLM', model,
            prompt:message, outputs:[],
            runMs:Math.round(performance.now() - t0),
            createdAt:Date.now(),
            request:{backend:provider || 'llm', model},
        });
        scheduleSave();
    } catch(e) {
        ...
        if(!Array.isArray(canvas.logs)) canvas.logs = [];
        canvas.logs.unshift({
            status:'failed', platform:node.llmProvider || 'LLM',
            model:node.llmModel || '',
            prompt:message, outputs:[],
            error:e.message || String(e),
            runMs:Math.round(performance.now() - t0),
            createdAt:Date.now(),
            request:{backend:node.llmProvider || 'llm'},
        });
        scheduleSave();
