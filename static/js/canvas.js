...
async function openCanvasFolder(){
    if(!canvas || !canvas.id) return;
    if(window.electronAPI?.isElectron){
        const folder = await window.electronAPI.selectFolder();
        if(folder) await window.electronAPI.openFolderInExplorer(folder);
        return;
    }
    setStatus('正在打开资源管理器...');
    try {
        const res = await fetch('/api/open-folder', {
            ...
            body: JSON.stringify({canvas_id: canvas.id})
        });
        if(!res.ok){
            const data = await res.json().catch(()=>({}));
            setStatus('打开文件夹失败: ' + (data.detail || res.status));
        } else {
            setTimeout(() => setStatus(''), 2000);
        }
    } catch(e) {
        console.error('openCanvasFolder error:', e);
    }
}
...
const openCanvasFolderBtn = document.getElementById('openCanvasFolderBtn');
...
const outputSaveAssetBtn = document.getElementById('outputSaveAssetBtn');
...
function handleRailClick(e){
    ...
    switch(type){
        ...
        case 'prompt': addPromptNode(); break;
        case 'loop': addLoopNode(); break;
        case 'llm': addLLMNode(); break;
        case 'generator': addGeneratorNode(); break;
        case 'msgen': addMsGenNode(); break;
        case 'video': addVideoNode(); break;
        case 'rh': addRhNode(); break;
        case 'comfy': addComfyNode(); break;
        case 'ltxDirector': addLTXDirectorNode(); break;
        case 'output': addOutputNode(); break;
        case 'group': groupSelectedImages(); break;
    }
}
function initRailLibrary(){
    ...
    if(panel){
        ...
        rail.addEventListener('mouseenter', function(){
            clearTimeout(libOpenTimer);
            panel.classList.add('is-open');
        });
        rail.addEventListener('mouseleave', function(){
            libOpenTimer = setTimeout(function(){
        ...
        panel.addEventListener('click', handleRailClick);
...
function setCanvasMode(open){
    ...
    if(!open){
        ...
        if(openCanvasFolderBtn) openCanvasFolderBtn.style.display = 'none';
    } else if(currentCanvasTitle) {
        currentCanvasTitle.textContent = canvas?.title || tr('canvas.untitled');
        currentCanvasTime.textContent = formatCanvasTime(canvas?.updated_at || canvas?.created_at);
        if(openCanvasFolderBtn) openCanvasFolderBtn.style.display = '';
    }
    refreshIcons();
}
...
function openImageNodeMenu(nodeId, clientX, clientY){
    ...
    imageNodeMenu.innerHTML = `
        ...
        <button class="menu-btn" data-image-save-asset="${escapeAttr(nodeId)}" ${canPreview ? '' : 'disabled'}><i data-lucide="archive" class="w-4 h-4"></i><span>保存到素材库</span></button>
    `;
    ...
    const saveAssetBtn = imageNodeMenu.querySelector('[data-image-save-asset]');
    if(saveAssetBtn){
        saveAssetBtn.onclick = e => {
            e.stopPropagation();
            closeImageNodeMenu();
            saveNodeImageToAssetLibrary(nodeId);
        };
    }
    refreshIcons();
}
...
function openOutputNodeMenu(nodeId, clientX, clientY){
    ...
    imageNodeMenu.innerHTML = `
        ...
        <button class="menu-btn" data-output-save-asset="${escapeAttr(nodeId)}" ${imageCount ? '' : 'disabled'}><i data-lucide="archive" class="w-4 h-4"></i><span>全部保存到素材库</span></button>
    `;
    const menuWidth = 260;
    ...
    const saveAssetBtn = imageNodeMenu.querySelector('[data-output-save-asset]');
    if(saveAssetBtn){
        saveAssetBtn.onclick = e => {
            e.stopPropagation();
            closeImageNodeMenu();
            saveOutputImagesToAssetLibrary(nodeId);
        };
    }
    refreshIcons();
}
...
function renderNode(node){
    ...
    if(node.type === 'image') {
        if(node.url) {
            ...
            body.innerHTML = `<div class="image-preview-wrap">${missing ? missingAssetHtml(node.url) : canvasPreviewImgHtml(node.url, 480, 'draggable="false"')}</div><div class="image-caption text-[11px] text-gray-400 truncate">${escapeHtml(node.name || 'image')}${missing ? ` · ${langIsEn() ? 'missing' : '文件缺失'}` : ''}</div>`;
...
function showCanvasAssetHoverPreview(event, item){
    ...
    if(img){
        ...
        img.src = canvasMediaPreviewUrl(isVideo ? item.url : (item.thumbnail || item.url || ''), 480);
...
async function _ensureAssetTarget(){
    if(canvasAssetLibraryIsLocal()){
        setStatus('本地素材请在素材库管理中上传');
        return null;
    }
    if(!canvasAssetLibrary.libraries?.length){
        const res = await fetch('/api/asset-library').catch(() => null);
        if(res?.ok) canvasAssetLibrary = (await res.json()).library || canvasAssetLibrary;
    }
    const libs = (canvasAssetLibrary.libraries || []).filter(l => l.id !== 'local');
    if(!libs.length){ setStatus('没有可用的资产库'); return null; }
    return new Promise(resolve => {
        const overlay = document.createElement('div');
        overlay.className = 'asset-folder-picker-overlay';
        ...
        card.style.cssText = 'background:var(--bg,#fff);border-radius:12px;padding:20px;min-width:320px;box-shadow:0 8px 32px rgba(0,0,0,.25);display:flex;flex-direction:column;gap:12px';
        overlay.onclick = e => { if(e.target === overlay){ document.body.removeChild(overlay); resolve(null); } };
        card.onclick = e => e.stopPropagation();

        const title = document.createElement('div');
        title.style.cssText = 'font-weight:700;font-size:14px;color:var(--text,#000)';
        title.textContent = '选择保存位置';

        const libLabel = document.createElement('label');
        libLabel.style.cssText = 'font-size:12px;color:var(--muted,#666)';
        libLabel.textContent = '资产库';

        const libSelect = document.createElement('select');
        libSelect.style.cssText = 'width:100%;padding:6px 8px;border:1px solid var(--line,#ddd);border-radius:6px;background:var(--input-bg,#f8f8f8);color:var(--text,#000);font-size:13px';
        libs.forEach((l, i) => {
            const opt = document.createElement('option');
            opt.value = l.id; opt.textContent = l.name || l.id;
            if(l.id === activeCanvasAssetLibraryId || (!activeCanvasAssetLibraryId && i === 0)) opt.selected = true;
            libSelect.appendChild(opt);
        });

        const catLabel = document.createElement('label');
        catLabel.style.cssText = 'font-size:12px;color:var(--muted,#666)';
        catLabel.textContent = '分组';

        const catSelect = document.createElement('select');
        catSelect.style.cssText = 'width:100%;padding:6px 8px;border:1px solid var(--line,#ddd);border-radius:6px;background:var(--input-bg,#f8f8f8);color:var(--text,#000);font-size:13px';

        function updateCats(){
            const lib = libs.find(l => l.id === libSelect.value);
            const cats = (lib?.categories || []).filter(c => String(c.type || 'image') !== 'workflow');
            catSelect.innerHTML = '';
            cats.forEach((c, i) => {
                const opt = document.createElement('option');
                opt.value = c.id; opt.textContent = c.name || c.id;
                if(c.id === activeCanvasAssetCategoryId || (!activeCanvasAssetCategoryId && i === 0)) opt.selected = true;
                catSelect.appendChild(opt);
            });
        }
        libSelect.onchange = updateCats;
        updateCats();
        ...
        cancelBtn.style.cssText = 'padding:6px 16px;border:1px solid var(--line,#ddd);border-radius:6px;background:var(--input-bg,#f8f8f8);color:var(--text,#000);font-size:13px;cursor:pointer';
        cancelBtn.onclick = () => { document.body.removeChild(overlay); resolve(null); };

        const confirmBtn = document.createElement('button');
        confirmBtn.textContent = '保存到此位置';
        confirmBtn.style.cssText = 'padding:6px 16px;border:none;border-radius:6px;background:var(--accent,#000);color:#fff;font-size:13px;cursor:pointer';
        confirmBtn.onclick = () => {
            const libId = libSelect.value;
            const catId = catSelect.value;
            activeCanvasAssetLibraryId = libId;
            activeCanvasAssetCategoryId = catId;
            document.body.removeChild(overlay);
            resolve({library_id:libId, category_id:catId});
        };
...
async function saveNodeImageToAssetLibrary(nodeId){
    const node = nodes.find(n => n.id === nodeId);
    if(!node?.url) return;
    const target = await _ensureAssetTarget();
    if(!target) return;
    setStatus('正在保存到素材库...');
    try {
        const item = await _uploadImageToAsset(target.library_id, target.category_id, node.url, node.name || 'image');
        if(item) setStatus('已保存到素材库');
    } catch(err) {
        setStatus(err.message || '保存失败');
    }
}
async function saveOutputImagesToAssetLibrary(nodeId){
    const node = nodes.find(n => n.id === nodeId);
    const urls = outputImageUrls(node);
    if(!node || !urls.length) return;
    const target = await _ensureAssetTarget();
    if(!target) return;
    const prompt = node.meta?.run?.prompt || '';
    setStatus(`正在保存 ${urls.length} 张图片到素材库...`);
    let saved = 0;
    for(const url of urls){
        try {
            const item = await _uploadImageToAsset(target.library_id, target.category_id, url, outputImageName(url) || 'image', prompt);
            if(item) saved++;
        } catch(e) {
            console.error('save output image failed:', url, e);
        }
    }
    renderCanvasAssetLibrary();
    setStatus(saved ? `已保存 ${saved}/${urls.length} 张图片到素材库` : '保存失败');
}
async function savePreviewToAssetLibrary(url, prompt, out){
    const target = await _ensureAssetTarget();
    if(!target) return;
    setStatus('正在保存到素材库...');
    try {
        const item = await _uploadImageToAsset(target.library_id, target.category_id, url, outputDownloadName(url) || 'image', prompt);
        if(item) setStatus('已保存到素材库');
    } catch(err) {
        setStatus(err.message || '保存失败');
    }
}
async function _uploadImageToAsset(libraryId, categoryId, url, name, caption){
    const resp = await fetch(url);
    if(!resp.ok) throw new Error('下载图片失败');
    ...
    const uploadRes = await fetch('/api/asset-library/items/upload', {method:'POST', body:form});
    if(!uploadRes.ok) throw new Error((await uploadRes.json().catch(()=>{})).detail || '上传失败');
    const uploadData = await uploadRes.json();
    const item = uploadData.item;
    if(caption && item?.id){
        await fetch(`/api/asset-library/items/${item.id}`, {
            method:'PATCH',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({caption})
        }).catch(()=>{});
    }
    await loadCanvasAssetLibrary({renderPanel:true});
    return item;
}
...
async function runLLMNode(nodeId, opts={}){
    ...
    try {
        ...
        refreshNodes([node.id]);
        scheduleSave();
        if(!Array.isArray(canvas.logs)) canvas.logs = [];
        canvas.logs.unshift({
            status:'success', platform:'LLM',
            prompt:input, model:node.model || node.llmProvider || '',
            outputs:[{text:node.outputText}],
            runMs:Math.round(performance.now() - t0),
            createdAt:Date.now(),
            request:{backend:node.llmProvider || 'llm', taskId:node.id},
        });
    } catch(err) {
        if(!opts.cascade) node.running = false;
        if(isCascadeAbortError(err)){
...
function openOutputLightbox(url, out){
    ...
    if(outputSaveAssetBtn){
        outputSaveAssetBtn.onclick = e => {
            e.stopPropagation();
            const prompt = currentOutputMeta?.run?.prompt || outputPromptText.textContent || '';
            savePreviewToAssetLibrary(currentOutputLightboxUrl, prompt, out);
        };
    }
...
function closeOutputLightbox(){
    ...
    if(outputSaveAssetBtn) outputSaveAssetBtn.onclick = null;
    resetOutputPreviewZoom();
