...
// import canvas: file input change
document.getElementById('importCanvasInput')?.addEventListener('change', function(){
    if(this.files?.length) importCanvasFile(this.files[0]);
    this.value = '';
});

// import canvas: drag & drop on workspace
const workspaceEl = document.getElementById('workspace');
let dragCounter = 0;
workspaceEl.addEventListener('dragenter', e => { e.preventDefault(); dragCounter++; workspaceEl.classList.add('dragover'); });
workspaceEl.addEventListener('dragleave', e => { e.preventDefault(); dragCounter--; if(!dragCounter) workspaceEl.classList.remove('dragover'); });
workspaceEl.addEventListener('dragover', e => { e.preventDefault(); });
workspaceEl.addEventListener('drop', e => {
    e.preventDefault(); dragCounter = 0; workspaceEl.classList.remove('dragover');
    const files = e.dataTransfer?.files;
    if(files?.length) importCanvasFile(files[0]);
});
