...
async function openFolder(key){
    ...
    try {
        ...
        if (!res.ok) {
            ...
            console.error('openFolder failed:', data.detail || res.status);
        }
    } catch(e) {
        ...
        <aside class="asset-panel asset-nav">
            <div class="panel-head">
                ...
                <div class="panel-actions compact-actions">
                    ${localFolderTreeEdit?.placement === 'head' ? '' : '<button class="asset-icon-btn" type="button" data-localup-folder-new title="新建文件夹"><i data-lucide="folder-plus"></i></button>'}
            ...
            ${localFolderTreeEdit?.placement === 'head' ? `
            <div class="head-inline-edit" style="flex:none;padding:0 12px 8px">
                <input id="localFolderTreeEditInput" type="text" value="新文件夹" placeholder="文件夹名称">
                <button type="button" class="primary" data-localfolder-tree-edit-save title="保存"><i data-lucide="check"></i><span>保存</span></button>
                <button type="button" data-localfolder-tree-edit-cancel title="取消"><i data-lucide="x"></i><span>取消</span></button>
            </div>` : ''}
            <div class="nav-scroll">
                <div class="nav-tree">
            ...
            <div class="detail-body">
                ...
                ${isImage ? `<div class="detail-caption-card">
                    <div class="detail-caption-head">
                        <strong>提示词</strong>
                        <div class="detail-caption-actions">
                            <button class="asset-btn" type="button" data-asset-caption-copy="${escapeAttr(item.id)}"><i data-lucide="copy"></i><span>复制</span></button>
                            <button class="asset-btn primary" type="button" data-asset-caption-save="${escapeAttr(item.id)}"><i data-lucide="save"></i><span>保存</span></button>
                        </div>
                    </div>
                    <textarea id="assetCaptionEdit" class="detail-caption-textarea" placeholder="暂无提示词">${escapeHtml(item.caption || '')}</textarea>
                </div>` : ''}
...
async function uploadFiles(files){
    ...
    const refreshed = await apiJson('/api/asset-library');
    assetLibrary = refreshed.library || assetLibrary;
    selectedAssetIds.clear();
...
async function handleClick(event){
    ...
    if(target.closest?.('[data-localup-folder-new]')){ localFolderTreeEdit = {placement:'head', value:'新文件夹', label:'文件夹名称'}; render(); focusTreeEditInput('localFolderTreeEditInput'); return; }
    ...
    if(target.closest?.('[data-localfolder-tree-edit-save]')){ await saveLocalFolderTreeEdit(); return; }
    if(target.closest?.('[data-localfolder-tree-edit-cancel]')){ localFolderTreeEdit = null; render(); return; }
    ...
    const assetCaptionSave = target.closest?.('[data-asset-caption-save]');
    if(assetCaptionSave){ await saveAssetCaption(assetCaptionSave.dataset.assetCaptionSave || ''); return; }
    const assetCaptionCopy = target.closest?.('[data-asset-caption-copy]');
    if(assetCaptionCopy){ await copyAssetCaption(assetCaptionCopy.dataset.assetCaptionCopy || ''); return; }
...
async function saveAssetCaption(id){
    const item = id ? findAssetItem(id) : selectedAsset();
    if(!item) return;
    const textarea = document.getElementById('assetCaptionEdit');
    const caption = textarea ? textarea.value : '';
    try {
        const data = await apiJson(`/api/asset-library/items/${encodeURIComponent(item.id)}`, {
            method:'PATCH',
            headers:{'Content-Type':'application/json'},
...
async function copyAssetCaption(id){
    const textarea = document.getElementById('assetCaptionEdit');
...
async function saveLocalFolderTreeEdit(){
    if(!localFolderTreeEdit) return;
    const name = document.getElementById('localFolderTreeEditInput')?.value || '';
    if(!String(name || '').trim()){
        setStatus('名称不能为空');
        return;
    }
    try {
        await apiJson('/api/local-assets/folders', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({parent:activeLocalUploadFolder || '', name})
        });
...
document.addEventListener('keydown', event => {
    ...
    if(event.target?.id === 'localFolderTreeEditInput'){
        if(event.key === 'Enter'){ event.preventDefault(); saveLocalFolderTreeEdit().catch(err => setStatus(err.message || '保存失败')); }
        if(event.key === 'Escape'){ event.preventDefault(); localFolderTreeEdit = null; render(); }
    }
});
...
openFolderBtn?.addEventListener('click', () => {
    const key = activeTab === 'canvas-assets' ? 'canvas-files' : activeTab;
    openFolder(key);
});
